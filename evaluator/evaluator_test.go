...
func testIntegerArrayObject(t *testing.T, obj object.Object, expected []int64) bool {
	...
	if len(expected) != len(array.Elements) {
	...
	for i, obj := range array.Elements {
		...
		if !ok {
			t.Errorf("integer is not object.Integer. got=%T", obj)
			return false
		}
		if integer.Value != expected[i] {
			t.Errorf("object has wrong value. got=%d, want=%d", integer.Value, expected[i])
			return false
		}
	}
...
func TestStrginConcatenation(t *testing.T) {
	input := `"Hello" + " " + "World!"`
...
func TestBuiltinFunctions(t *testing.T) {
	tests := []struct {
		...
		{`len([]);`, 0},
		{`len(["one", "two", "three", "four", "five"]);`, 5},
		{`first([]);`, NULL},
		{`first([1,2,3]);`, 1},
		{`let array = [1, 2, 3]; first(array);`, 1},
		{`first(-1);`, "argument to `first` must be ARRAY. got INTEGER"},
		{`first([3, 2, 1], 2);`, "wrong number of arguments. got=2, want=1"},
		{`last([]);`, NULL},
		{`last([1,2,3]);`, 3},
...
func TestHashLiterals(t *testing.T) {
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("Hash has wrong number of pair. got=%d, want=%d", len(result.Pairs), len(expected))
	}
